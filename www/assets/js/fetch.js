...
var strapi = new _main2.default("http://api:1337");

console.log("Running.");
console.log(strapi);

strapi.login("admin", "password").then(function () {
  strapi.getEntries("aboutpage").then(function (aboutPageElements) {
    console.log("aboutPageElements count: " + aboutPageElements.length);
    aboutPageElements.map(function (elem) {
      (0, _jquery2.default)("#" + elem.ElementId).text(elem.TextValue);
    });
  });
});

var strapi2 = new _main2.default("http://localhost:1337");
console.log("Running2.");
console.log(strapi2);

strapi2.login("admin", "password").then(function () {
  strapi2.getEntries("aboutpage").then(function (aboutPageElements) {
    console.log("aboutPageElements count: " + aboutPageElements.length);
    aboutPageElements.map(function (elem) {
      (0, _jquery2.default)("#" + elem.ElementId).text(elem.TextValue);
    });
...
var sdk_1 = __importDefault(__webpack_require__(70));
...
Object.defineProperty(exports, "__esModule", { value: true });
var axios_1 = __importDefault(__webpack_require__(32));
var Cookies = __importStar(__webpack_require__(68));
var qs = __importStar(__webpack_require__(71));
...
var Strapi = function () {
    ...
    function Strapi(baseURL, storeConfig, requestConfig) {
        ...
        this.axios = axios_1.default.create(Object.assign({ baseURL: baseURL, paramsSerializer: qs.stringify }, requestConfig));
    ...
    _createClass(Strapi, [{
        key: "request",
        value: async function request(method, url, requestConfig) {
            try {
                var response = await this.axios.request(Object.assign({ method: method,
        ...
        key: "register",
        value: async function register(username, email, password) {
            this.clearToken();
            var authentication = await this.request('post', '/auth/local/register', {
                data: {
                    email: email,
                    password: password,
                    username: username
        ...
        key: "login",
        value: async function login(identifier, password) {
            this.clearToken();
            var authentication = await this.request('post', '/auth/local', {
                data: {
        ...
        key: "forgotPassword",
        value: async function forgotPassword(email, url) {
            this.clearToken();
        ...
        key: "resetPassword",
        value: async function resetPassword(code, password, passwordConfirmation) {
            this.clearToken();
        ...
        value: async function authenticateProvider(provider, params) {
            this.clearToken();
            // Handling browser query
            if (this.isBrowser()) {
                params = qs.parse(window.location.search, { ignoreQueryPrefix: true });
            }
            var authentication = await this.request('get', "/auth/" + provider + "/callback", {
                params: params
            });
            this.setToken(authentication.jwt);
            return authentication;
        ...
        value: function upload(data, requestConfig) {
            return this.request('post', '/upload', Object.assign({ data: data }, requestConfig));
        }
        /**
